...
#define MAXLINESZ 1024 /* max line size */
...
char sbuf[MAXLINESZ];    /* for composing sprintf messages */

struct job_t {               /* The job struct */
    pid_t pid;               /* job PID */
    int jid;                 /* job ID [1, 2, ...] */
    int state;               /* UNDEF, BG, FG, or ST */
    char cmdline[MAXLINESZ]; /* command line */
};
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if ((fgets(cmdline, MAXLINESZ, stdin) == NULL) && ferror(stdin))
 ...
 */
int parseline(const char *cmdline, char **argv) {
    static char array[MAXLINESZ]; /* holds local copy of command line */
